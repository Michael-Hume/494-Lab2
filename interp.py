...
fname = input("File: ")
inputFile = open( fname ) # out_rest_2.txt
inputFile.readline()
inputFile.readline()
...
def collect():
    ...
    while True:
        ...
        br = [ sum(x)/3.0 for x in brf ]
        hb = [ sum(x)/3.0 for x in hbf ]
...
interp(rawBR, rawHR, med=False)

if input("stress? ") != 'y': exit(1)

data = []
try: data.append( collect() )
except: pass
try: data.append( collect() )
except: pass
...
interp(br1, hb1, med=True)
interp(br2, hb2, med=True)
